...
#if [ -d $ARQUIVO_DIR -a $TEMP_DIR ]; then
#	echo "Diretorio $ARQUIVO_DIR e $TEMP_DIR criados com sucesso"
#fi
...
find $LOG_DIR -name "*.log" -print0 | while IFS= read -r -d '' arquivo; do
        ...
        echo "Arquivo: $nome_arquivo" >> "${ARQUIVO_DIR}/logs_stats_$(date +%F).txt"
        echo "Número de linhas: $num_linhas" >> "${ARQUIVO_DIR}/logs_stats_$(date +%F).txt"
        echo "Número de palavras: $num_palavras" >> "${ARQUIVO_DIR}/logs_stats_$(date +%F).txt"
        echo "--------------------------" >> "${ARQUIVO_DIR}/logs_stats_$(date +%F).txt"
